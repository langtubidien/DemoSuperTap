...
import ltbd.group.demosupertap.Base.BaseFragment;
import ltbd.group.demosupertap.databinding.FragmentProductBinding;
...
public class HomeFragment extends BaseFragment implements HomeContract.View {
    ...
    protected void beforeSetContentView() {
        ...
        binding = FragmentProductBinding.inflate(activity.getLayoutInflater());
        initView();
        addAction();
        initData();
    }
    ...
    private void initView() {
        ...
        binding.rcyFeatured.setLayoutManager(new GridLayoutManager(activity, 3));
        featureAdapter = new ProductAdapter(featuredProducts);
        binding.rcyFeatured.setAdapter(featureAdapter);

        binding.rcyProduct.setLayoutManager(new GridLayoutManager(activity, 3));
        productAdapter = new ProductAdapter(products);
        binding.rcyProduct.setAdapter(productAdapter);

        binding.rcyNewFuntion.setLayoutManager(new LinearLayoutManager(activity, RecyclerView.HORIZONTAL, false));
    ...
    private void addAction() {
        binding.tvPosition.setOnClickListener(v -> {
            Intent intent = new Intent(activity, MapContainerActivity.class);
    ...
    private void setUpInformationFormLongLat(double longT, double latT) {
        activity.runOnUiThread(() -> binding.tvPosition.setText(getAddressFormLatLong(longT, latT)));
    }
